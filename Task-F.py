...
##### Вариант 2 (он работает медленнее, но и памяти занимает меньше)
#
# Принцип решения. Сформируем список svazi: в нём будут находиться множества (сеты), 
# содержащие связанные между собой вершины графа. Каждый такой сет от других сетов 
# будет изолирован. Далее останется проверить, что склад-заказчик и склад-поставщик 
# находятся в одном и том же сете (т.е. индексы их сетов в списке svazi совпадают).
